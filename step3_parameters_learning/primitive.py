...
def clip(parameters, coordinates):
    ...
    orientation = torch.tensor([0.,0,1])
    width = torch.abs(parameters[0])
    height = torch.abs(parameters[1])
    thickness = torch.abs(parameters[2])
    dist = torch.abs(parameters[3])
    open_gap = torch.abs(parameters[4])
    
    # Calculate the angle
    angle = torch.atan((open_gap - dist) / height / 2.0)
    ...
    rot_1 = rotation_matrix(-angle) 
    rot_2 = rotation_matrix(angle)

    def rotate_and_translate(points, rot_matrix):
        return torch.matmul(points - center, rot_matrix.T) + center

    difference = torch.tensor([(dist + open_gap) / 4.0, 0., 0])

    # Generate faces for the first cuboid (parallel to each other)
    shorten_scale = height/2 + abs(height / (dist + open_gap) - height / 2) # these two only works when open_gap = 0
    shiftup = torch.tensor([0., 0, (height - shorten_scale) / 2], dtype=torch.float32)
    
    if abs(-height * (dist + open_gap) / (dist - open_gap) / 2) > height / 2:
        shiftup = torch.tensor([0., 0, 0.], dtype=torch.float32)
        

    points_front_1 = generate_face_points(coords_front_1, z_dir, y_dir, half_thickness * x_dir.unsqueeze(0), shorten_scale, width)
    points_back_1 = generate_face_points(coords_back_1, z_dir, y_dir, -half_thickness * x_dir.unsqueeze(0), height, width)

    # Translate the first cuboid faces
    points_front_1 = rotate_and_translate(points_front_1 + shiftup, rot_1) + difference
    points_back_1 = rotate_and_translate(points_back_1, rot_1) + difference # this is the right cuboid, right face

    # Generate faces for the second cuboid (parallel to each other)
    points_front_2 = generate_face_points(coords_front_2, z_dir, y_dir, half_thickness * x_dir.unsqueeze(0), height, width)
    points_back_2 = generate_face_points(coords_back_2, z_dir, y_dir, -half_thickness * x_dir.unsqueeze(0), shorten_scale, width)

    # Rotate and translate the second cuboid faces
    points_front_2 = rotate_and_translate(points_front_2, rot_2) - difference
    points_back_2 = rotate_and_translate(points_back_2 + shiftup, rot_2) - difference   # this is the left cuboid,  right face
