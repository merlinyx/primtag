...
void Non_DraggedView_Seg(igl::opengl::glfw::Viewer& viewer, const vector<Eigen::Vector3d>& points) {
    ...
    std::string filePath = "../../../segmented_" + selected_option + "_mesh.obj";
...
if (points.empty()) {
    ...
    string filePath = "../../../volumetric_" + selected_option + "_mesh.mesh";
...
void DraggedView_Seg(igl::opengl::glfw::Viewer& viewer, const vector<Eigen::Vector3d>& points) {
    ...
    string filePath = "../../../segmented_" + selected_option + "_mesh.obj";
    if (!igl::writeOBJ(filePath, viewer.data().V, F_new)) {
...
int main(int argc, char *argv[])
{
    ...
    {
        // Dropdown menu
        const char* options[] = { "rod", "hemisphere", "hook", "hole", "edge", "surface", "tube", "clip" };
        static int current_item = 0;
        if (ImGui::Combo("Segmentation Options", &current_item, options, IM_ARRAYSIZE(options))) {
            selected_option = options[current_item];
